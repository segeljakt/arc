...
//{Int32BufferBuilder, Float32BufferBuilder};
use arrow::buffer::Buffer; use arrow::array::*;
use arrow::datatypes::Float32Type as F32;
...
fn main() -> Result<()> {
    ...
    // storage data
    let tensor = Tensor::<I32>::try_new(buf, Some(vec![2, 8]), None, None)?;
    ...
    // The new function will estimate the expected stride for the
    // storage data
    let tensor = Tensor::<F32>::try_new(buf, Some(vec![2, 2]), None, None)?;
    println!("\nFloat32 Tensor");
    println!("{:?}", tensor);
    ...
    let tensor = Tensor::<I32>::try_new(buf, Some(vec![2, 5]), None, None)?;
    println!("\nInt32 Tensor");
    println!("{:?}", tensor);

...
pub(crate) struct Channel<T: Data> {
...
pub fn channel<T: Data>(sys: &KompactSystem) -> (Pushable<T>, Pullable<T>) {
    let chan = sys.create(Channel::new);
    sys.start(&chan);
    (Pushable(chan.actor_ref()), Pullable(chan.actor_ref(), 1))
}

impl<T: Data> ComponentLifecycle for Channel<T> {}

#[derive(Debug)]
pub(crate) enum Message<T: Data> {
    PushRequest(Ask<T, ()>),
...
impl<T: Data> Actor for Channel<T> {
...
pub struct Pushable<T: Data>(pub(crate) ActorRef<Message<T>>);

pub struct Pullable<T: Data>(pub(crate) ActorRef<Message<T>>, pub(crate) usize);

impl<T: Data> Clone for Pushable<T> {
    fn clone(&self) -> Self {
...
impl<T: Data> Drop for Pushable<T> {
    fn drop(&mut self) {
        self.0.tell(Message::DelPusher);
    }
}

impl<T: Data> Drop for Pullable<T> {
    fn drop(&mut self) {
